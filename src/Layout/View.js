...
import { Route, Switch, Link, useParams, useHistory, useLocation } from "react-router-dom"
        ...
        <Switch>
            ...
            <div key={card.id} className="card">
                <div className="card-body">
                    ...
                    <button type="button" onClick={() => (window.confirm("Delete this card?")?(deleteCard(card.id),history.push(`/decks/${deck.id}`)):null)} className="btn btn-danger"><FontAwesomeIcon icon={faTrashAlt} /></button>
                </div>
            </div>))}
            </Route>
        </Switch>
